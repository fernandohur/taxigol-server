...
class TokensControllerTest < ActionController::TestCase
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post :create, token: { company_id: @token.company_id, customer: @token.customer, value: @token.value }
    end
  ...
  test "should update token" do
    put :update, id: @token, token: { company_id: @token.company_id, customer: @token.customer, value: @token.value }
    assert_redirected_to token_path(assigns(:token))
  end
